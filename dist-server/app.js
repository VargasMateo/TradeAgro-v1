...
function generateInviteToken(uuid) {
    return createHmac('sha256', INVITE_SECRET)
        .update(uuid)
        .digest('hex')
        .substring(0, 16);
}
function validateInviteToken(uuid, token) {
...
const authenticateToken = (req, res, next) => {
    ...
    jwt.verify(token, JWT_SECRET, async (err, user) => {
        ...
        try {
            ...
            const [userRows] = await pool.query('SELECT isTest FROM users WHERE id = ?', [user.id]);
...
const seedDefaultUsers = async (connection = pool) => {
    ...
    try {
        ...
        // 1. Admin
        const [adminCheck] = await connection.query('SELECT id FROM users WHERE email = ?', ['admin@tradeagro.com']);
        if (adminCheck.length === 0) {
            console.log('[SEED] Inserting Admin...');
            await connection.query('INSERT INTO users (displayName, email, password, role, createdBy) VALUES (?, ?, ?, ?, ?)', ['Admin TradeAgro', 'admin@tradeagro.com', adminPass, 'admin', 1]);
        }
        // 2. Profesional
        const [profCheck] = await connection.query('SELECT id FROM users WHERE email = ?', ['profesional@tradeagro.com']);
        if (profCheck.length === 0) {
            console.log('[SEED] Inserting Profesional...');
            const [profRes] = await connection.query('INSERT INTO users (displayName, email, password, role, createdBy) VALUES (?, ?, ?, ?, ?)', ['Juan Tecnico', 'profesional@tradeagro.com', profPass, 'profesional', 1]);
            await connection.query('INSERT INTO profesionals (userId, specialty, phoneNumber) VALUES (?, ?, ?)', [profRes.insertId, 'Ingeniero Agrónomo - Especialista en Riego', '5491155551234']);
        }
        // 3. Cliente
        const [clientCheck] = await connection.query('SELECT id FROM users WHERE email = ?', ['cliente@tradeagro.com']);
...
const seedDefaultServices = async (connection = pool) => {
    try {
...
async function initializeDatabase() {
    ...
    `);
        ...
        const [idCol] = await connection.query('SHOW COLUMNS FROM users LIKE "id"');
        ...
        const [createdByCol] = await connection.query('SHOW COLUMNS FROM users LIKE "createdBy"');
    ...
    `);
        ...
        try {
            const [columns] = await connection.query('SHOW COLUMNS FROM profesionals LIKE "phone"');
            if (columns.length > 0) {
    ...
    `);
        ...
        const [fieldIdCol] = await connection.query('SHOW COLUMNS FROM fields LIKE "id"');
    ...
    `);
        ...
        try {
            ...
            if (e.code !== 'ER_DUP_FIELDNAME')
                console.error('[INIT] secondaryService migration error:', e.message);
        }
        ...
        try {
            ...
            if (e.code !== 'ER_DUP_FIELDNAME')
                console.error('[INIT] groupId migration error:', e.message);
        }
    ...
    `);
        ...
        try {
            const [columns] = await connection.query('SHOW COLUMNS FROM work_order_attachments');
            const hasFileData = columns.some((c) => c.Field === 'fileData');
            if (!hasFileData) {
            ...
            const hasDisplayOrder = columns.some((c) => c.Field === 'displayOrder');
            if (!hasDisplayOrder) {
    ...
    `);
        ...
        try {
            const [columns] = await connection.query('SHOW COLUMNS FROM work_orders');
            const hasUserId = columns.some((c) => c.Field === 'userId');
            const hasJobCode = columns.some((c) => c.Field === 'jobCode');
            const hasCreatedBy = columns.some((c) => c.Field === 'createdBy');
            const hasDeletedAt = columns.some((c) => c.Field === 'deletedAt');
            const hasFieldId = columns.some((c) => c.Field === 'fieldId');
            const hasUuid = columns.some((c) => c.Field === 'uuid');
            if (!hasUuid) {
            ...
            if (!hasCreatedBy) {
                ...
                const [createdBySpec] = await connection.query('SHOW COLUMNS FROM work_orders LIKE "createdBy"');
            ...
            const [idColumn] = await connection.query('SHOW COLUMNS FROM work_orders LIKE "id"');
        ...
        try {
            const [tables_old_campos] = await connection.query("SHOW TABLES LIKE 'tbl_campos'");
        ...
        try {
            const [tables_old] = await connection.query("SHOW TABLES LIKE 'tbl_trabajos'");
            ...
            const [tables_pascal] = await connection.query("SHOW TABLES LIKE 'WorkOrders'");
        ...
        try {
            ...
            const hasParentId = columns.some((c) => c.Field === 'parentId');
            if (!hasParentId) {
            ...
            const [indexes] = await connection.query('SHOW INDEX FROM services WHERE Column_name = "name" AND Non_unique = 0');
            if (indexes.length > 0) {
...
app.put('/backend/profile', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (role === 'profesional') {
            ...
            await connection.query('UPDATE clients SET businessName = ?, cuit = ?, ivaCondition = ?, phoneNumber = ?, notificationEmails = ?, hasSprayMonitor = ? WHERE userId = ?', [businessName || null, cuit || null, ivaCondition || 'Responsable Inscripto', phoneNumber || null, notificationEmails || null, hasSprayMonitor === undefined ? null : !!hasSprayMonitor, id]);
        }
...
async function sendPasswordSetupEmail(userEmail, displayName, token) {
...
async function sendForgotPasswordEmail(userEmail, displayName, token) {
    const transporter = getTransporter();
    if (!transporter) {
        console.warn('[EMAIL] SMTP not configured. Skipping email send.');
        throw new Error('Servidor de correo no configurado');
...
async function sendOrderCompletedEmail(orderData) {
    const transporter = getTransporter();
    if (!transporter) {
...
async function createPasswordSetupToken(connection, userId) {
    ...
    await connection.query('UPDATE password_setup_tokens SET usedAt = NOW() WHERE userId = ? AND usedAt IS NULL', [userId]);
    await connection.query('INSERT INTO password_setup_tokens (userId, token, expiresAt) VALUES (?, ?, ?)', [userId, token, expiresAt]);
    return token;
}
// ==========================================
...
apiRouter.post('/auth/setup-password', async (req, res) => {
    ...
    `, [token]);
        ...
        await connection.query('UPDATE users SET password = ? WHERE id = ?', [hashedPassword, tokenRow.userId]);
        // Mark token as used
        await connection.query('UPDATE password_setup_tokens SET usedAt = NOW() WHERE id = ?', [tokenRow.id]);
        await connection.commit();
...
apiRouter.post('/auth/forgot-password', async (req, res) => {
    ...
    try {
        // 1. Find user
        const [users] = await connection.query('SELECT id, displayName, email FROM users WHERE email = ?', [email]);
        if (users.length === 0) {
...
apiRouter.post('/auth/resend-invite', authenticateToken, async (req, res) => {
    ...
    const connection = await pool.getConnection();
    try {
        const [userRows] = await connection.query('SELECT id, displayName, email FROM users WHERE id = ?', [userId]);
        if (userRows.length === 0) {
...
apiRouter.put('/clients/:id', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const [fieldsRows] = await pool.query('SELECT * FROM fields WHERE clientId = ?', [userId]);
        const formattedFields = fieldsRows.map((f) => ({
            ...f,
...
apiRouter.post('/work-orders', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (!finalClientId && req.body.client) {
            const [userRows] = await pool.query('SELECT id FROM users WHERE displayName = ? AND role = "client" LIMIT 1', [req.body.client]);
            if (userRows.length > 0) {
        ...
        if (notes && notes.trim()) {
            ...
            await pool.query('INSERT INTO work_order_observations (workOrderId, userId, text) VALUES (?, ?, ?)', [result.insertId, creatorId, notes.trim()]);
        }
        res.json({
            success: true,
            id: dbData.uuid
...
apiRouter.patch('/work-orders/:id/status', authenticateToken, async (req, res) => {
    ...
    try {
        // Resolve UUID to internal numeric ID
        const [woRows] = await pool.query('SELECT id, clientId, profesionalId FROM work_orders WHERE uuid = ? AND deletedAt IS NULL', [id]);
        if (woRows.length === 0) {
            return res.status(404).json({ success: false, error: 'Orden de trabajo no encontrada' });
        }
...
apiRouter.get('/work-orders/:id', authenticateToken, async (req, res) => {
    ...
    `;
        ...
        if (row.groupId) {
            const [relatedRows] = await pool.query('SELECT uuid, lotName FROM work_orders WHERE groupId = ? AND uuid != ? AND deletedAt IS NULL', [row.groupId, id]);
            job.relatedOrders = relatedRows;
        }
        else {
            job.relatedOrders = [];
        }
 ...
 */
apiRouter.get('/work-orders/:id/invite-token', authenticateToken, async (req, res) => {
    const { id } = req.params;
    const user = req.user;
    try {
        const [rows] = await pool.query('SELECT uuid, clientId, profesionalId FROM work_orders WHERE uuid = ? AND deletedAt IS NULL', [id]);
        if (rows.length === 0) {
            return res.status(404).json({ success: false, error: 'Orden de trabajo no encontrada' });
        }
        const row = rows[0];
 ...
 */
apiRouter.post('/work-orders/:id/attachments', authenticateToken, upload.array('files'), async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        const [maxOrderRows] = await pool.query('SELECT COALESCE(MAX(displayOrder), -1) as maxOrder FROM work_order_attachments WHERE workOrderId = ?', [internalJobId]);
        ...
        await pool.query('INSERT INTO work_order_attachments (workOrderId, fileName, fileUrl, fileType, fileSize, fileData, uploadedBy, description, displayOrder) VALUES ?', [values]);
...
apiRouter.get('/work-orders/:id/attachments', authenticateToken, async (req, res) => {
    ...
    try {
        // Resolve UUID to internal numeric ID
        const [woRows] = await pool.query('SELECT id, clientId, profesionalId, groupId FROM work_orders WHERE uuid = ? AND deletedAt IS NULL', [id]);
        if (woRows.length === 0) {
            return res.status(404).json({ success: false, error: 'Orden de trabajo no encontrada' });
        }
        const order = woRows[0];
 ...
 */
apiRouter.put('/attachments/update-meta', authenticateToken, async (req, res) => {
...
apiRouter.get('/work-orders/:id/observations', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const internalJobId = order.id;
        const [rows] = await pool.query(`SELECT o.id, o.workOrderId, o.userId, o.text, o.createdAt,
              u.displayName, u.role
       FROM work_order_observations o
       JOIN users u ON o.userId = u.id
       WHERE o.workOrderId = ?
       ORDER BY o.createdAt ASC`, [internalJobId]);
        res.json(rows);
    }
    catch (error) {
 ...
 */
apiRouter.post('/work-orders/:id/observations', authenticateToken, async (req, res) => {
    const { id } = req.params;
    const userId = req.user.id;
    const { text } = req.body;
    if (!text || !text.trim()) {
    ...
    try {
        ...
        const internalJobId = order.id;
        const [result] = await pool.query('INSERT INTO work_order_observations (workOrderId, userId, text) VALUES (?, ?, ?)', [internalJobId, userId, text.trim()]);
...
apiRouter.get('/attachments/:id/content', authenticateToken, async (req, res) => {
    const { id } = req.params;
...
apiRouter.delete('/attachments/:id', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const [rows] = await pool.query('SELECT uploadedBy FROM work_order_attachments WHERE id = ?', [attachmentId]);
        if (rows.length === 0) {
        ...
        const [result] = await pool.query('DELETE FROM work_order_attachments WHERE id = ?', [attachmentId]);
...
apiRouter.post('/fields', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const [result] = await pool.query('INSERT INTO fields SET ?', [dbData]);
...
apiRouter.get('/attachments', authenticateToken, async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM work_order_attachments');
        res.json(rows);
    }
    catch (error) {
...
apiRouter.get('/observations', authenticateToken, async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM work_order_observations');
        res.json(rows);
    }
    catch (error) {
...
apiRouter.get('/tokens', authenticateToken, async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM password_setup_tokens');
        res.json(rows);
    }
    catch (error) {
...
apiRouter.patch('/clients/:id/spray-monitor-toggle', authenticateToken, async (req, res) => {
    ...
    const connection = await pool.getConnection();
    try {
        await connection.query('UPDATE clients SET hasSprayMonitor = ? WHERE userId = ?', [!!hasSprayMonitor, id]);
...
apiRouter.post('/profesionales', authenticateToken, async (req, res) => {
    ...
    try {
       ...
       WHERE u.email = ?`, [email]);
        ...
        await connection.query(`INSERT INTO profesionals (userId, phoneNumber, specialty, deletedAt)
       VALUES (?, ?, ?, NULL)
       ON DUPLICATE KEY UPDATE
       phoneNumber = VALUES(phoneNumber), specialty = VALUES(specialty), deletedAt = NULL`, [newUserId, phoneNumber || null, specialty || null]);
 ...
 */
apiRouter.get('/public/work-orders/:uuid', async (req, res) => {
    const { uuid } = req.params;
    const { token } = req.query;
    if (!token || !validateInviteToken(uuid, token)) {
    ...
    `;
        ...
        const [attachmentRows] = await pool.query(`SELECT id, fileName, fileType, fileSize, displayOrder, description, createdAt
       FROM work_order_attachments
       WHERE workOrderId = ?
       ORDER BY displayOrder ASC, createdAt DESC`, [row.id]);
 ...
 */
apiRouter.get('/public/attachments/:id', async (req, res) => {
    const { id } = req.params;
    ...
    `, [id]);
        ...
        if (!validateInviteToken(uuid, token)) {
